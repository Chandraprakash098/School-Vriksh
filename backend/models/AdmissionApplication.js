...
const admissionApplicationSchema = new mongoose.Schema({
  ...
  paymentStatus: { type: String, enum: ['not_applicable', 'pending', 'completed'], default: 'pending' },
  paymentDetails: {
    transactionId: String,
    amount: Number,
    paidAt: Date,
...
admissionApplicationSchema.methods.validateDocuments = function () {
  if (this.admissionType === 'RTE') {
    ...
    return requiredDocs.every((docType) => this.documents.some((doc) => doc.type === docType));
  }
};

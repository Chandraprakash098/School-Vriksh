...
generateFeeSlip : async (student, payment, feesPaid, schoolId, monthYear) => {
  const currentDate = new Date();
  const receiptNumber = payment.receiptNumber || `REC${Date.now()}`;
  const feeSlipId = `FS-${receiptNumber}`;
  ...
  // Upload to S3
  const fileKey = `fee_receipts/receipt_${feeSlipId}.pdf`;
  let pdfUrl;
  try {
    pdfUrl = await uploadToS3(pdfBuffer, fileKey, "application/pdf");
  } catch (error) {
    logger.error(`Failed to upload fee receipt to S3: ${error.message}`, {
      error,
    });
    throw new Error(`Failed to upload fee receipt: ${error.message}`);
  }

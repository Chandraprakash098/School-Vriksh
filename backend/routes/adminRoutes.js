...
// router.get('/students/class/:classId', auth,adminController.getStudentsByClass);
...
// Announcement Management
router.post('/announcements', auth, roleCheck(['admin']), announcementUpload.array('files'), adminController.createAnnouncement);
router.get('/announcements', auth, roleCheck(['admin']), adminController.getAnnouncements);
router.get('/announcements/:id', auth, roleCheck(['admin']), adminController.getAnnouncementById);
router.put('/announcements/:id', auth, roleCheck(['admin']), announcementUpload.array('files', 5), adminController.updateAnnouncement);
router.delete('/announcements/:id', auth, roleCheck(['admin']), adminController.deleteAnnouncement);
...
// Leave Management
router.get('/leave-requests/pending', [auth, roleCheck(['admin'])], adminController.getPendingLeaveRequests);
router.put('/leave-requests/:leaveId/review', [auth, roleCheck(['admin'])], adminController.reviewLeaveRequest);
router.get('/leave-requests/history', [auth, roleCheck(['admin'])], adminController.getLeaveRequestHistory);
router.delete('/leave-requests/:leaveId', [auth, roleCheck(['admin'])], adminController.deleteLeaveRequest);

// Trustee Management
router.put('/trustees/:trusteeId', auth, roleCheck(['admin']), adminController.manageTrustee);

// Exam Schedule and Results Management
router.post('/schedule', [auth, roleCheck(['admin'])], adminController.createExamSchedule);
router.get('/schedules', auth, roleCheck(['admin']), adminController.getExamSchedules);

// Updated Exam Results Workflow
router.get('/exams/:examId/classes/:classId/review', [auth, roleCheck(['admin'])], adminController.reviewClassResults);
router.post('/exams/:examId/classes/:classId/publish', [auth, roleCheck(['admin'])], adminController.publishResults);
router.get('/exams/:examId/classes/:classId/report-cards', [auth, roleCheck(['admin'])], adminController.generateReportCards);
router.get('/exams/:examId/classes/:classId/performance-metrics', [auth, roleCheck(['admin'])], adminController.trackPerformanceMetrics);

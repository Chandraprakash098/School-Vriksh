...
payFeesForStudent: async (req, res) => {
  try {
    ...
    for (const fee of selectedFees) {
      ...
      // Check for missing fee types
      const definedTypes = feeDefinitions.map(f => f.type);
      const missingTypes = types.filter(t => !definedTypes.includes(t));
      if (missingTypes.length > 0) {
        return res.status(400).json({
          message: `Fee types not defined for ${month}/${year}: ${missingTypes.join(', ')}`,
        });
      }

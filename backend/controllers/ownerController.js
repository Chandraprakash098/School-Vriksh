...
const ownerController = {
  ...
  registerSchool: async (req, res) => {
    try {
      ...
      const logoFile = req.file; // This will be set by multer middleware
      ...
      const school = new School({
        ...
        logoKey: logoFile ? logoFile.key : null // Store S3 key if logo is uploaded
      });
      await school.save();
      ...
      res.status(201).json({ 
        ...
        school: {
          ...
          logoUrl: logoFile ? logoFile.location : null // Include logo URL in response
        },
        admin: { ...admin.toObject(), password: undefined }
      });
    } catch (error) {
